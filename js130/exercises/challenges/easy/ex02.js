...
class DNA {
  ...
  trimToSameLength(otherSequence) {
    let [shortest, longest] = [this.sequence, otherSequence].sort((a, b) => a.length - b.length);
    let targetLength = shortest.length;
    let trimmedLongest = longest.slice(0, targetLength);
    return [shortest, trimmedLongest];
  }

  hammingDistance(otherSequence) {
    let [seq1, seq2] = this.trimToSameLength(otherSequence);
